...
The default model assumes passenger mutations only. The value `max_fitness`
gives the maximum value that the fitness can achieve (defined as birth_rate
minus death rate) so that every mutation after has no functional effect on
fitness.
...
| max_fitness   | numeric                 | lower bound to fitness distribution|
